...
const PLAYER_WEAPONS =
    Object.freeze({
        NORMAL:
            window.PersonajeNormal.id,

        SHOTGUN:
            window.PersonajeEscopeta.id,

        SWORD:
            window.PersonajeEspada.id
    });
...
// Cerberon: oleadas 5, 15, 25...
loadSprite(
    "arena2",
    `${SPRITES_PATH}Arena2.png`
);

// Nexus Prime: oleadas 10, 20, 30...
loadSprite(
    "arena3",
...
if (!window.NeonExperience) {
    throw new Error(
        "Carga SistemaExperiencia.js antes de main.js"
    );
}


window.NeonExperience
    .initialize(
        player
    );
...
function drawPlayer() {
    const shootingDuration = 420;
    const hitDuration = 160;
    const deathDuration = 2400;
...
function drawWeaponHUD() {
    // El cambio de arma con Q continúa funcionando,
    // pero el texto no aparece en pantalla.
}
...
function registerEnemyDefeat(
    ...
    // Evitar que el mismo enemigo
    // sea contado varias veces.
...
function updateEnemigos() {
    for (
        ...
        // Algunos enemigos pueden morir
        // dentro de update(), como Kamikaze.
...
function shoot() {
    ...
    /*
     * Los módulos reciben x/y
     * como esquina superior izquierda.
     *
     * En este motor player.x y player.y
    ...
    // Aplicar mejoras de experiencia.
...
// ============================================================
// COLISIÓN DE PROYECTILES CONTRA ENEMIGOS
// ============================================================
...
function checkProjectileEnemyCollisions() {
    for (
        ...
        // Un proyectil enemigo no puede
        // golpear a otro enemigo.
...
function damagePlayer(amount) {
    ...
    // La resistencia reduce como máximo
    // 50% del daño recibido.
    ...
    if (
        ...
        // Detener música y reproducir muerte.
        ...
        // Registrar puntuación una sola vez.
...
function gameLoop() {
    ...
    // --------------------------------------------------------
    // COMPROBAR SELECCIÓN DE MEJORA
    // --------------------------------------------------------

    const experienceAllowsGameplay =
        !window.NeonExperience ||
        !window.NeonExperience
            .isChoosingUpgrade();
